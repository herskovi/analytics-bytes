...
public class SendSmsController extends BaseController
{
	...
	public void execute() throws Exception
	{
		...
		try
		{
			...
			if (cust != null)
			{
				...
				profileName = custUtils.getProfileName();
				...
				try 
				{
					...
					if (!isLocalMode) 
					{ 
						IController smsFlowController = new NexmoSmsController(cust, textMessage.toString());							
	...
	private void addTextMessageHeader(Customer cust) {
		textMessage.delete(0, textMessage.length());
		textMessage.append("Hi " + cust.getName() + ",\n");
	}
