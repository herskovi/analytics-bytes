...
//			getServletContext().getRequestDispatcher(RequestDispatcherConsts.FROM_PAY_PAL_TO_ORDER_CONFIRMATION).forward(req, resp);
			writeJsonObject(smsConfirmationResponse.getMessage(), resp);
		} catch (Exception e) 
		{
	 ...
	 *@Date:        Apr 13, 2015
	 *@Description: Write Json Object
	 */
	private void writeJsonObject(String message, HttpServletResponse resp) {
		Gson gson = new Gson();
		...
		try {
			...
			log.severe("SMSConfirmationServlet writeJsonObject failed " +  e.getMessage());
		}		
